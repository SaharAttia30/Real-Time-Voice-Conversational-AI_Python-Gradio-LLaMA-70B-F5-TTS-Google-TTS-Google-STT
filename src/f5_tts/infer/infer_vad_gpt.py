...
with gr.Blocks(title=APP_TITLE, theme=gr.themes.Soft()) as app:
    ...
    def vad_stream_processor(stream_tuple, vad_state):
        ...
        # ... (Keep the initial checks and stream end handling as before) ...
        ...
        # --- Unpack Stream & CONVERT TO MONO ---
        try:
            if not isinstance(stream_tuple, tuple) or len(stream_tuple) != 2: raise ValueError("Incorrect stream format")
            sample_rate, stream_chunk_raw = stream_tuple # Raw chunk, potentially stereo
            if stream_chunk_raw is None or not isinstance(stream_chunk_raw, np.ndarray): raise ValueError("Invalid audio data")

            print(f"[VAD Stream] Original numpy chunk shape: {stream_chunk_raw.shape}") # Log original shape

            # --- Convert Stereo to Mono by averaging channels ---
            if stream_chunk_raw.ndim == 2:
                # Shape (samples, 2) or (2, samples)? Assume (samples, 2) is common
                if stream_chunk_raw.shape[1] == 2:
                    print(f"[VAD Stream] Converting stereo chunk {stream_chunk_raw.shape} to mono...")
                    stream_chunk_mono = stream_chunk_raw.mean(axis=1) # Average across channels
                elif stream_chunk_raw.shape[0] == 2: # Handle (2, samples) case
                    print(f"[VAD Stream] Converting stereo chunk {stream_chunk_raw.shape} to mono...")
                    stream_chunk_mono = stream_chunk_raw.mean(axis=0) # Average across channels
                else:
                    raise ValueError(f"Audio chunk has unexpected 2D shape: {stream_chunk_raw.shape}")
            elif stream_chunk_raw.ndim == 1:
                # Already mono
                stream_chunk_mono = stream_chunk_raw
            else:
                raise ValueError(f"Audio chunk has unexpected dimensions: {stream_chunk_raw.ndim}")

            print(f"[VAD Stream] Mono numpy chunk shape: {stream_chunk_mono.shape}") # Should always be 1D now
        ...
        # --- Check VAD Iterator ---
        # ... (Keep VAD iterator check as before) ...
        if not vad_state.get("vad_iterator"):
            # ... (Reset logic) ...
            return vad_state, gr.update(value=update_status), trigger_update # Return after reset


        # --- Process Audio Chunk (Convert Mono, Resample, Pad, VAD) ---
        try:
            # 1. Convert 1D MONO numpy chunk to 1D tensor
            chunk_tensor_orig_sr = torch.from_numpy(stream_chunk_mono.astype(np.float32)) # Use the mono version

            # 2. Resample if necessary (Resample preserves 1D shape)
            # ... (Resampling logic using chunk_tensor_orig_sr -> audio_chunk_for_buffer, same as before) ...
            resampler = None
            ...
            # ... (Padding logic using audio_chunk_for_buffer -> chunk_for_vad, same as before) ...
            if audio_chunk_for_buffer.shape[0] < min_vad_samples:
                # ... calculate padding ...
                padding = torch.zeros(min_vad_samples - audio_chunk_for_buffer.shape[0], dtype=audio_chunk_for_buffer.dtype, device=audio_chunk_for_buffer.device)
                chunk_for_vad = torch.cat((audio_chunk_for_buffer, padding))
            else:
                chunk_for_vad = audio_chunk_for_buffer


            # --- 4. Run VAD (using the 1D tensor) ---
            # ... (VAD call using chunk_for_vad, same as before) ...
            print(f"Shape passed to VAD: {chunk_for_vad.shape}, dtype: {chunk_for_vad.dtype}") # Check it's 1D
            if chunk_for_vad.ndim != 1: raise ValueError(f"Tensor shape passed to VAD is not 1D: {chunk_for_vad.shape}")
            if chunk_for_vad.shape[0] < min_vad_samples: print(f"ERROR: Chunk ({chunk_for_vad.shape[0]}) too small for VAD (min: {min_vad_samples})"); speech_dict = None
            else: speech_dict = vad_state["vad_iterator"](chunk_for_vad, return_seconds=True)


        except Exception as e:
            # ... (Error handling for processing chunk) ...
        ...
        # ... (VAD logic based on speech_dict, buffering audio_chunk_for_buffer, triggering - same as before) ...
        if speech_dict:
