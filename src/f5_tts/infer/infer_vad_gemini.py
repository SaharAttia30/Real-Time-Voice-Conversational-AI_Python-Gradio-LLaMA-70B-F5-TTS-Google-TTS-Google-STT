...
VAD_CHUNK_SIZE = 512  # Expected chunk size for 16kHz
VAD_THRESHOLD = 0.5
VAD_MIN_SILENCE_DURATION_MS = 700 # Used by VADIterator state, not chunk processing
...
""" # Add the rest of your Example Flow if desired
...
with gr.Blocks(title=APP_TITLE, theme=gr.themes.Soft()) as app:
    ...
    # NOTE: sample_rate argument removed as it caused errors for the user. RESAMPLING IS NOW USED.
    audio_input_stream = gr.Audio(
        label="Microphone Input (Live VAD when enabled)",
        sources=["microphone"], streaming=True, type="numpy",
        # sample_rate=16000, # Removed based on user feedback - resampling needed now
    ...
    def reset_vad_state_logic(current_vad_state):
        ...
        else: current_vad_state["vad_iterator"] = None
        ...
        # Do not reset resampler here, it depends on incoming sample rate
    ...
    def process_speech_segment(audio_segment_path, current_history, current_conv_state):
        ...
        # Proceed only if transcription successful
    ...
    # --- VAD Streaming Callback (Includes Resampling and Padding) ---
   # --- VAD Constants ---
    VAD_SAMPLE_RATE = 16000
    VAD_CHUNK_SIZE = 512  # Expected chunk size for 16kHz
    VAD_THRESHOLD = 0.5
    VAD_MIN_SILENCE_DURATION_MS = 700 # Used by VADIterator state, not chunk processing
    VAD_SPEECH_PAD_MS = 300         # Used by VADIterator state, not chunk processing

    # --- Revised VAD Streaming Callback ---
    def vad_stream_processor(stream_tuple, vad_state):
        update_status = "Status: Listening..."
        trigger_update = gr.update() # Path for triggering transcription

        # --- Initial Checks & VAD Readiness ---
        if not VAD_ENABLED or VAD_MODEL is None:
            return vad_state, gr.update(value="Status: VAD Disabled"), trigger_update
        if vad_state["processing_active"]: # Prevent processing new audio while busy transcribing/responding
            return vad_state, gr.update(value="Status: Processing Previous..."), trigger_update
        if not vad_state.get("vad_iterator"):
            print("[VAD] Iterator not ready, resetting.")
            vad_state, update_status = reset_vad_state_logic(vad_state)
            if not vad_state.get("vad_iterator"):
                print("ERROR: VAD Iterator Initialization failed.")
                return vad_state, gr.update(value="Status: VAD Error"), trigger_update
            # Ensure input buffer is initialized correctly after reset
            if "input_audio_buffer" not in vad_state or not isinstance(vad_state["input_audio_buffer"], torch.Tensor):
                vad_state["input_audio_buffer"] = torch.empty(0, dtype=torch.float32)

        # --- Stream End Handling ---
        if stream_tuple is None:
            print(" Audio stream ended.")
            # Process any remaining audio in the input buffer
            if vad_state["input_audio_buffer"].shape > 0:
                print(f" Processing residual input buffer: {vad_state['input_audio_buffer'].shape} samples")
                remaining_samples = vad_state["input_audio_buffer"]
                vad_state["input_audio_buffer"] = torch.empty(0, dtype=torch.float32) # Clear buffer
                # Pad the final segment to VAD_CHUNK_SIZE
                if remaining_samples.shape < VAD_CHUNK_SIZE:
                    padding = torch.zeros(VAD_CHUNK_SIZE - remaining_samples.shape, dtype=remaining_samples.dtype)
                    final_chunk_for_vad = torch.cat((remaining_samples, padding))
                else: # Should ideally not happen if loop handles chunks >= 512, but safety check
                    final_chunk_for_vad = remaining_samples

                try:
                    # Process the final padded chunk through VAD
                    speech_dict = vad_state["vad_iterator"](final_chunk_for_vad, return_seconds=True)
                    if speech_dict and "start" in speech_dict and not vad_state["is_speaking"]:
                        print(" Speech Start detected")
                        vad_state["is_speaking"] = True
                        # Append the original remaining samples (not padded) if speech starts here
                        vad_state["buffer"].append(remaining_samples)
                        vad_state["silence_start_time"] = None
                    elif speech_dict and "end" in speech_dict and vad_state["is_speaking"]:
                        print(" Speech End detected")
                        # Append the original remaining samples before marking end
                        vad_state["buffer"].append(remaining_samples)
                        vad_state["is_speaking"] = False
                        # Trigger processing immediately as stream has ended
                        if vad_state["buffer"] and not vad_state["processing_active"]:
                            vad_state["processing_active"] = True
                            update_status = "Status: Processing Final Segment..."
                            print(" Triggering processing for final speech segment.")
                            try:
                                full_speech_tensor = torch.cat(vad_state["buffer"])
                                with tempfile.NamedTemporaryFile(suffix=".wav", delete=False) as tmp_f:
                                    sf.write(tmp_f.name, full_speech_tensor.numpy(), VAD_SAMPLE_RATE)
                                    print(f"Saved final segment: {tmp_f.name}")
                                    trigger_update = gr.update(value=tmp_f.name)
                            except Exception as e:
                                print(f"ERROR saving final buffer: {e}"); traceback.print_exc()
                                vad_state["processing_active"] = False; update_status = "Status: Error Saving Final Audio"
                            vad_state["buffer"] = []
                        else:
                            print(" Speech ended, but buffer empty or already processing.")
                            vad_state["buffer"] = []
                    elif vad_state["is_speaking"]:
                        # If speech was ongoing, append the remaining samples
                        vad_state["buffer"].append(remaining_samples)

                except Exception as e:
                    print(f"Error processing residual VAD input: {e}"); traceback.print_exc()
                    try: vad_state["vad_iterator"].reset_states()
                    except: pass

            # Handle case where speech was ongoing but stream ended without a VAD 'end' event on the residual
            if vad_state["is_speaking"] and vad_state["buffer"] and not vad_state["processing_active"]:
                print(" Stream ended while speaking, processing accumulated buffer.")
                vad_state["processing_active"] = True
                update_status = "Status: Processing Final Segment..."
                try:
                    full_speech_tensor = torch.cat(vad_state["buffer"])
                    with tempfile.NamedTemporaryFile(suffix=".wav", delete=False) as tmp_f:
                        sf.write(tmp_f.name, full_speech_tensor.numpy(), VAD_SAMPLE_RATE)
                        print(f"Saved final segment (stream end): {tmp_f.name}")
                        trigger_update = gr.update(value=tmp_f.name)
                except Exception as e:
                    print(f"ERROR saving final buffer (stream end): {e}"); traceback.print_exc()
                    vad_state["processing_active"] = False; update_status = "Status: Error Saving Final Audio"
                vad_state["buffer"] = []
                vad_state["is_speaking"] = False # Mark as not speaking anymore

            print(" End processing complete.")
            # Reset VAD iterator state for potential future streams if app stays open
            try: vad_state["vad_iterator"].reset_states()
            except: pass
            return vad_state, gr.update(value="Status: Stream Ended"), trigger_update

        # --- Process Incoming Audio Chunk ---
        try:
            if not isinstance(stream_tuple, tuple) or len(stream_tuple)!= 2: raise ValueError("Incorrect stream format")
            sample_rate, stream_chunk = stream_tuple
            if stream_chunk is None or not isinstance(stream_chunk, np.ndarray): raise ValueError("Invalid audio data")
            audio_tensor = torch.from_numpy(stream_chunk.astype(np.float32))
            if audio_tensor.ndim > 1: audio_tensor = torch.mean(audio_tensor, dim=1)
            if audio_tensor.ndim!= 1: raise ValueError(f"Audio tensor is not 1D after processing (shape: {audio_tensor.shape})")
            chunk_tensor_orig_sr = audio_tensor

            # Resample if necessary
            if sample_rate!= VAD_SAMPLE_RATE:
                if vad_state.get("resampler") is None or vad_state.get("resampler_orig_freq")!= sample_rate:
                ...
                resampler = vad_state["resampler"]
                audio_chunk_for_buffer = resampler(chunk_tensor_orig_sr)
            else:
                audio_chunk_for_buffer = chunk_tensor_orig_sr

            # Append preprocessed chunk to the input buffer
            vad_state["input_audio_buffer"] = torch.cat((vad_state["input_audio_buffer"], audio_chunk_for_buffer), dim=0)

        except Exception as e:
            print(f"Error preparing or buffering audio tensor: {e}"); traceback.print_exc()
            return vad_state, gr.update(value="Status: Stream Error"), trigger_update

        # --- Process Input Buffer in Fixed Chunks ---
        processed_chunk_in_loop = False
        try:
    
            while vad_state["input_audio_buffer"].shape[0] >= VAD_CHUNK_SIZE:
                vad_chunk = vad_state["input_audio_buffer"][:VAD_CHUNK_SIZE]
                vad_state["input_audio_buffer"] = vad_state["input_audio_buffer"][VAD_CHUNK_SIZE:]
                process_vad_chunk(vad_chunk, vad_state)
                processed_chunk_in_loop = True
                # Extract one chunk of the correct size
                vad_input_chunk = vad_state["input_audio_buffer"]
                # Update the input buffer (remove the processed chunk)
                vad_state["input_audio_buffer"] = vad_state["input_audio_buffer"]

                # --- Call VAD on the 512-sample chunk ---
                # print(f"[VAD Process Loop] Feeding chunk shape: {vad_input_chunk.shape}") # Debug print
                speech_dict = vad_state["vad_iterator"](vad_input_chunk, return_seconds=True)

                # --- VAD Logic (Applied per 512-sample chunk) ---
                if speech_dict:
                    if "start" in speech_dict and not vad_state["is_speaking"]:
                        print(f"[VAD] Speech Start detected @ {speech_dict['start']:.2f}s (within chunk)")
                        vad_state["is_speaking"] = True
                        # Start buffering detected speech. Add the current chunk.
                        vad_state["buffer"] = [vad_input_chunk]
                        vad_state["silence_start_time"] = None
                        update_status = "Status: User Speaking..."
                    elif "end" in speech_dict and vad_state["is_speaking"]:
                        print(f"[VAD] Speech End detected @ {speech_dict['end']:.2f}s (within chunk)")
                        # Append the final chunk of this speech segment
                        vad_state["buffer"].append(vad_input_chunk)
                        vad_state["is_speaking"] = False
                        vad_state["silence_start_time"] = time.time() # Use system time for silence tracking post-detection

                        # --- Trigger Transcription ---
                        if vad_state["buffer"] and not vad_state["processing_active"]:
                            vad_state["processing_active"] = True # Block further VAD processing until transcription done
                            update_status = "Status: Processing Speech..."
                            print("[VAD] Triggering processing for detected speech segment...")
                            try:
                                # Concatenate buffered speech tensors
                                full_speech_tensor = torch.cat(vad_state["buffer"])
                                # Save to temp file
                                with tempfile.NamedTemporaryFile(suffix=".wav", delete=False) as tmp_f:
                                    sf.write(tmp_f.name, full_speech_tensor.numpy(), VAD_SAMPLE_RATE)
                                    print(f"Saved segment for transcription: {tmp_f.name}")
                                    # Update the hidden textbox to trigger the.change() event
                                    trigger_update = gr.update(value=tmp_f.name)
                            except Exception as e:
                                print(f"ERROR saving speech buffer: {e}"); traceback.print_exc()
                                vad_state["processing_active"] = False # Release lock on error
                                update_status = "Status: Error Saving Audio"
                        else:
                            print("[VAD] Speech ended, but buffer empty or already processing.")
                        # Clear the speech buffer after attempting to process it
                        vad_state["buffer"] = []
                    elif vad_state["is_speaking"]:
                        # Ongoing speech, append the current chunk to the speech buffer
                        vad_state["buffer"].append(vad_input_chunk)
                        update_status = "Status: User Speaking..."
                        vad_state["silence_start_time"] = None # Reset silence timer if speech continues

                elif vad_state["is_speaking"]:
                    # VAD detects silence, but we were previously speaking. Append chunk (might contain trailing sound).
                    vad_state["buffer"].append(vad_input_chunk)
                    # Check for silence duration using VADIterator's state or system time if needed,
                    # but the 'end' event from VADIterator is the primary trigger.
                    # For simplicity here, we rely on the 'end' event.
                    update_status = "Status: User Speaking... (Potential Pause)"

            # Update status only if no chunks were processed (indicates just listening)
            if not processed_chunk_in_loop:
                update_status = "Status: Listening..."

        except Exception as e:
            print(f"Error during VAD processing loop: {e}"); traceback.print_exc()
            try: vad_state["vad_iterator"].reset_states() # Attempt VAD reset on error
            except: pass
            return vad_state, gr.update(value="Status: VAD Processing Error"), trigger_update

        # Return updated state, status message, and potentially the path to trigger transcription
    ...
    def handle_vad_trigger(audio_segment_path, current_history, current_conv_state, current_vad_state):
        ...
        # Reset processing flag AFTER segment handled, ready for next VAD trigger *or* audio gen
        current_vad_state["processing_active"] = False
        status_update = "Status: Generating AI Response..."
        # Return state to allow generate_audio_for_response to run next
    ...
    def toggle_vad(vad_choice, current_vad_state):
        ...
        if vad_choice and VAD_ENABLED:
             ...
             new_state, status = reset_vad_state_logic(current_vad_state)
             # Make mic stream interactive and visible
             ...
             status = "Status: VAD Disabled"
              # Mic stream non-interactive, but keep visible if VAD *could* load
    ...
    vad_enabled_toggle.change(
        ...
        queue=False # Make toggle responsive
    )

    # Connect the audio stream only if VAD was successfully enabled
    if VAD_ENABLED:
        ...
        # Chain: VAD Trigger -> Process Segment -> Generate Audio -> Reset VAD
        vad_trigger_path.change(
             ...
             reset_vad_state, # Reset VAD state after AI response audio is generated/sent
             inputs=[vad_internal_state],
             outputs=[vad_internal_state, status_display]
        )

    # Text input processing chain
    send_text_btn.click(
         text_input_pipeline,
    ...
    # Clear and Prompt Update handlers
