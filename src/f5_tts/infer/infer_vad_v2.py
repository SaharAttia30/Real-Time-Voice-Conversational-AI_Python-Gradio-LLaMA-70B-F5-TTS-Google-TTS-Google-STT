...
with gr.Blocks(title=APP_TITLE, theme=gr.themes.Soft()) as app:
    ...
    def vad_stream_processor(stream_tuple, vad_state):
        update_status = "Status: Listening..."; trigger_update = gr.update()
        min_vad_samples = 512 # Minimum samples required by Silero VAD at 16kHz

        # --- Initial Checks & Stream End Handling ---
        if not VAD_ENABLED or VAD_MODEL is None: return vad_state, gr.update(value="Status: VAD Disabled"), trigger_update
        if stream_tuple is None:
            # ... (Stream end handling - same as before) ...
            ...
            if vad_state["is_speaking"] and vad_state["buffer"] and not vad_state["processing_active"]:
                ...
                vad_state["processing_active"] = True; update_status = "Status: Processing residual..."
                try:
                    full_speech_tensor = torch.cat(vad_state["buffer"]) # Buffer contains 1D tensors
                    with tempfile.NamedTemporaryFile(suffix=".wav", delete=False) as tmp_f:
                        sf.write(tmp_f.name, full_speech_tensor.numpy(), VAD_SAMPLE_RATE); trigger_update = gr.update(value=tmp_f.name)
                except Exception as e: print(f"ERROR saving residual: {e}"); vad_state["processing_active"] = False; update_status = "Status: Error processing residual"
                vad_state["buffer"] = []; vad_state["is_speaking"] = False
            else: update_status = "Status: Listening..."
            return vad_state, gr.update(value=update_status), trigger_update

        if vad_state["processing_active"]: return vad_state, gr.update(value="Status: Processing..."), trigger_update

        # --- Unpack Stream & Check/Prepare Data ---
        try:
            ...
            # Log original shape
            # print(f"[VAD Stream] Original numpy chunk shape: {stream_chunk.shape}, dtype: {stream_chunk.dtype}")

            # Ensure chunk is 1D (remove channel dimension if present, e.g., (N, 1) -> (N,))
            if stream_chunk.ndim > 1:
                print(f"[VAD Stream] Squeezing chunk from {stream_chunk.shape}...")
                stream_chunk = np.squeeze(stream_chunk) # SQUEEZE NUMPY ARRAY
                # print(f"[VAD Stream] Squeezed chunk shape: {stream_chunk.shape}")

            if stream_chunk.ndim != 1:
                  raise ValueError(f"Audio chunk is not 1D after squeeze (shape: {stream_chunk.shape})")

        except Exception as e:
            print(f"Error unpacking/preparing stream chunk: {e}"); traceback.print_exc()
            return vad_state, gr.update(value="Status: Stream Error"), trigger_update

        # --- Check VAD Iterator ---
        if not vad_state.get("vad_iterator"):
            print("[VAD] Iterator not ready, resetting."); vad_state, update_status = reset_vad_state_logic(vad_state)
            if not vad_state.get("vad_iterator"): print("ERROR: VAD Init failed."); return vad_state, gr.update(value="Status: VAD Error"), trigger_update
            return vad_state, gr.update(value=update_status), trigger_update

        # --- Process Audio Chunk (Convert, Resample, Pad, VAD) ---
        try:
            # 1. Convert 1D numpy chunk to 1D tensor
            chunk_tensor_orig_sr = torch.from_numpy(stream_chunk.astype(np.float32)) # Should be 1D

            # 2. Resample if necessary (Resample preserves 1D shape)
            resampler = None
            if sample_rate != VAD_SAMPLE_RATE:
                ...
                audio_chunk_for_buffer = resampler(chunk_tensor_orig_sr) # Use resampled (1D) for buffer
            else:
                audio_chunk_for_buffer = chunk_tensor_orig_sr # Use original (1D) for buffer

            # 3. Pad 1D chunk *for VAD input* if it's too short
            if audio_chunk_for_buffer.shape[0] < min_vad_samples:
                padding_needed = min_vad_samples - audio_chunk_for_buffer.shape[0]
                padding = torch.zeros(padding_needed, dtype=audio_chunk_for_buffer.dtype, device=audio_chunk_for_buffer.device)
                chunk_for_vad = torch.cat((audio_chunk_for_buffer, padding)) # Padded 1D
            else:
                chunk_for_vad = audio_chunk_for_buffer # Original 1D

            # --- 4. Run VAD (using the 1D tensor) ---
            print(f"Shape passed to VAD: {chunk_for_vad.shape}, dtype: {chunk_for_vad.dtype}") # Should be (N,)
            if chunk_for_vad.ndim != 1:
                 # Defensive check - should pass now
                 raise ValueError(f"Tensor shape passed to VAD is not 1D: {chunk_for_vad.shape}")

            # Ensure minimum size (check before calling VAD)
            if chunk_for_vad.shape[0] < min_vad_samples:
                 print(f"ERROR: Chunk ({chunk_for_vad.shape[0]}) cannot be passed to VAD (min: {min_vad_samples}) despite padding!")
                 speech_dict = None # Skip VAD call for this chunk
            else:
                 # Call VAD with the 1D tensor
                 speech_dict = vad_state["vad_iterator"](chunk_for_vad, return_seconds=True)

        except Exception as e:
            print(f"Error during VAD processing chunk (Convert/Resample/Pad/VAD): {e}"); traceback.print_exc()
        ...
        # --- VAD Logic (Buffering the 1D tensor: audio_chunk_for_buffer) ---
        if speech_dict:
            if "start" in speech_dict and not vad_state["is_speaking"]:
                print(f"[VAD] Speech Start"); vad_state["is_speaking"] = True; vad_state["buffer"] = [audio_chunk_for_buffer]; vad_state["silence_start_time"] = None; update_status = "Status: User Speaking..."
            elif "end" in speech_dict and vad_state["is_speaking"]:
                print(f"[VAD] Speech End"); vad_state["buffer"].append(audio_chunk_for_buffer); vad_state["is_speaking"] = False; vad_state["silence_start_time"] = time.time()
                if vad_state["buffer"] and not vad_state["processing_active"]:
                    vad_state["processing_active"] = True; update_status = "Status: Processing Speech..."; print("[VAD] Triggering processing...")
                    try:
                        # Concatenate 1D tensors in buffer
                        full_speech_tensor = torch.cat(vad_state["buffer"])
                        with tempfile.NamedTemporaryFile(suffix=".wav", delete=False) as tmp_f:
                            # Save 1D tensor data
                            sf.write(tmp_f.name, full_speech_tensor.numpy(), VAD_SAMPLE_RATE); print(f"Saved segment: {tmp_f.name}"); trigger_update = gr.update(value=tmp_f.name)
                    except Exception as e: print(f"ERROR saving buffer: {e}"); vad_state["processing_active"] = False; update_status = "Status: Error Saving Audio"
                else: print("[VAD] Speech ended, buffer empty or already processing.")
                vad_state["buffer"] = [] # Clear buffer after processing attempt
            elif vad_state["is_speaking"]:
                 vad_state["buffer"].append(audio_chunk_for_buffer) # Ongoing speech

        elif vad_state["is_speaking"]:
            vad_state["buffer"].append(audio_chunk_for_buffer) # Buffer potential pauses
            update_status = "Status: User Speaking... (Pause?)"

        return vad_state, gr.update(value=update_status), trigger_update

        # --- VAD Logic (Buffering the original/resampled 1D chunk: audio_chunk_for_buffer) ---
        if speech_dict:
            if "start" in speech_dict and not vad_state["is_speaking"]:
                print(f"[VAD] Speech Start"); vad_state["is_speaking"] = True; vad_state["buffer"] = [audio_chunk_for_buffer]; vad_state["silence_start_time"] = None; update_status = "Status: User Speaking..."
            elif "end" in speech_dict and vad_state["is_speaking"]:
