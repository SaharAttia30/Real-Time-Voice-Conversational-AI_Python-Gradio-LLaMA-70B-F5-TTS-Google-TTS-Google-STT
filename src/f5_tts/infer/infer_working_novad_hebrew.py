...
import soundfile as sf  # Added for Google STT to get audio sample rate
import base64
import requests
import librosa
...
"Hebrew": """אתה עוזר AI עבור משרד עורכי הדין זמירה צדוק בנהריה. תחילה, שאל את הלקוח אם הוא גבר או אישה כדי להשתמש בצורות המגדר הנכונות בעברית. לאחר מכן, אסוף בקצרה את המידע הבא בלבד:

1. שם מלא של הלקוח או הלקוחה
2. מספר טלפון ליצירת קשר

**כללי שיחה:**
- היה מקצועי, קצר ואדיב.
- שאל שאלה אחת בכל פעם.
- אל תספק ייעוץ משפטי או תבקש פרטים משפטיים מורכבים.
- אשר את הפרטים שנאספו (שם, טלפון).
- סיים את השיחה באומר: "תודה, [שם]. נציג ממשרד עורכי הדין זמירה צדוק ייצור איתך קשר בהקדם האפשרי."

**דוגמת זרימה:**
אני: "שלום, זה העוזר AI ממשרד עורכי הדין זמירה צדוק. כדי ליפנות אליך נכון, האם אתה גבר או אישה?"
לקוח/ה: "אישה."
אני: "תודה. מה שמך המלא בבקשה?"
לקוח/ה: [מספקת שם]
אני: "מה מספר הטלפון שלך ליצירת קשר?"
לקוח/ה: [מספקת טלפון]
אני: "רק כדי לאשר: השם שלך הוא [שם], ומספר הטלפון הוא [טלפון]. הכל נכון?"
לקוח/ה: "כן."
...
# Define language codes for Google STT
language_codes = {
    "English": "en-US",
    "Hebrew": "he-IL"
}

def google_stt(audio_path, api_key, language_code):
    ...
    try:
        ...
        slow_down_rate = 0.8  # 0.8 means 80% of original speed (slower)
        data_stretched = librosa.effects.time_stretch(data, rate=slow_down_rate)
        print(f"Original audio length: {len(data)} samples")
        print(f"Stretched audio length: {len(data_stretched)} samples")

        with io.BytesIO() as buffer:
            sf.write(buffer, data_stretched, sample_rate, format="WAV", subtype="PCM_16")
            buffer.seek(0)
...
with gr.Blocks(title=APP_TITLE, theme=gr.themes.Soft()) as app:
    ...
    def process_user_input(
        audio_path, text_input, selected_language, current_history, conv_state_list
        ):
        ...
        if audio_path:
            ...
            try:
                # Use Google STT instead of preprocess_ref_audio_text
                transcribed_text = google_stt(audio_path, GOOGLE_API_KEY, language_codes[selected_language])
    ...
    process_inputs = [
        audio_input_chat,
        text_input_chat,
        language_chat,  # Added to pass selected language
        chatbot_interface,
        conversation_state
    ]
    process_outputs = [
